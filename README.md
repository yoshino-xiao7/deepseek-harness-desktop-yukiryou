...
| 功能 | 状态 | 计划范围 |
| --- | --- | --- |
| DeepSeek 宠物 | **开发中** | 当前 Beta 已内置经过深层校验的 YukiRyou 鲸鱼女仆开发预览，包含闲置、犯困、趴睡、唤醒揉眼与运行进食状态；设置中提供宠物资产库，Companion 顶部 Pet Stage 支持最小可见宽度、拖拽缩放和独立完全隐藏。普通用户最终只需角色参考图和自然语言，无需配置额外模型 Key；正式宠物通过视觉验收且格式稳定后再开放本地导入与制作 Skill。 |
