...
- 正式发行新增 macOS 与 Windows 真实跨版本自动升级门禁：从 GitHub 当前上一公开版开始，经正式更新 provider 下载精确候选、触发安装、确认旧进程退出、由操作系统拉起新版本，并再次执行打包应用冒烟验证。
- 跨版本门禁动态选择上一公开稳定版，不再依赖写死的历史版本；Windows 普通候选构建保持快速，只有正式发行才执行完整升级闭环。
