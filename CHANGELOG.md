...
- 修复升级至 Harness `0.1.5-rc.2` 后，Codex 插件提示“暂时无法连接本机授权服务”、DeepSeek 账户余额无法读取等第三方插件接口失效问题。
- 为宿主 Connection 补齐 WebServer 依赖，使插件 RPC 路由正常注册；保留浏览器认证与请求来源校验。
- 增加真实随包 Connection 的回归测试，覆盖第三方路由注册、未认证请求拒绝及插件卸载后的路由释放。
...
- 补齐外部插件更新检查结果、未挂载插件清单与接管更新提示；更新停用插件保持原停用状态。
- 新增宿主安全启动与持久化恢复预算；已识别加载失败的插件可隔离并逐个试启用，无法定位时跳过用户插件与补丁，保留原数据目录。
...
- 主页更新入口改用向下下载箭头；下载更新时订阅原生更新器进度，主页显示整数百分比，“关于”设置页同步显示真实百分比与确定型进度条，无法取得总大小时继续显示不确定型下载状态。
...
- lint、类型检查、485 项单元测试、48 项集成测试及 macOS 本地打包通过；Windows 候选的安装、启动、修复安装、再次启动和卸载生命周期门禁通过。
- 修复旧版本升级门禁直接改写 localStorage 后立即关闭旧版造成的会话选择竞态；门禁现在重载 Harness 并确认真实活动会话已生效后才关闭旧版，同时可复用既有候选单独验证这一关。
