...
- receipt 不匹配时拒绝卸载；外部插件只有在顶层包名、版本、入口和仓库回链均重新核验后，才可经原生确认进入受管接管。
