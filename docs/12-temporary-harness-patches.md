...
- 适用 Harness：`0.1.0-rc.8`
- 原因：rc.8 为官方 DeepSeek adapter 新增了 `inputModalities`，但 `llm-pi-ai` 自定义模型设置页仍未提供既有 `models[].input` 控件。
...
升级 Harness 时必须重新检查官方模型设置页。版本保护会拒绝把本补丁自动套用到非 `0.1.0-rc.8` 的 bundle；官方覆盖 `models[].input` 后再删除补丁模块、vendor 步骤与集成契约测试。
