...
- Windows 发行适配已进入 Phase 6：Forge 使用同一无扩展名品牌图标基址按目标解析 `.icns`/`.ico`，在 `win32` 同时生成 Squirrel 安装 EXE 与便携 ZIP，并提供 x64 package/make 命令、七尺寸 ICO 资产及可重复生成脚本。主进程在 Windows 提前处理 Squirrel 安装/更新/卸载事件，并设置与 package ID、可执行文件匹配的 AppUserModelID；Runtime schema 2 已纳入官方 Windows x64 Node ZIP、ConPTY 原生资产及 PE x64 校验。`Windows x64 candidate` CI 会在真实 Windows runner 上执行质量门禁、host-native Runtime vendor/verify、EXE/ZIP 打包、打包目录与便携 ZIP 启动/重启、会话恢复，并冻结 `Setup.exe`、portable ZIP、full nupkg、`RELEASES`、SHA-256 与提交来源清单；随后真实执行 Squirrel 首装、已安装应用启动、同版本修复安装、卸载、注册项/快捷方式清理和用户数据保留门禁。旧 Squirrel 的 `.dead`、`Update.exe`、`squirrel.exe` 与 V8 snapshot 自清理墓碑列入精确已知残留，其他残留一律失败。公开 Release 只选择版本化安装 EXE、便携 ZIP 和 Windows SHA-256 清单，NUPKG/RELEASES 留在 CI。当前 Windows Beta 有意保持未签名并明确披露 SmartScreen 风险；真正的上一版覆盖升级、自动更新闭环和独立 Windows 11 客户端实机验收继续完善，Authenticode 留待用户规模需要时接入。
