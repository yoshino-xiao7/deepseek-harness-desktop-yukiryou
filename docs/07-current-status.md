...
- 桌面端不再内置账户余额、余额 RPC、preload bridge 或账户概览开关。DeepSeek 余额与官方充值入口迁移到独立的 `dsh-deepseek-account` 插件；Grok 与 Codex 的使用额度分别由各自 provider 插件展示。三个插件都注册同一侧栏区域，但组件仅在当前会话选择自身 provider 时渲染；无法识别的自定义 provider 由 DeepSeek 插件兜底。
