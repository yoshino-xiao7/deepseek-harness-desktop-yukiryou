...
- 固定 Node.js 24.19.0、`@deepseek-ai/dsh` 0.1.5-rc.2、pnpm 10.34.5 及其校验值；拆分后的全部非可选 Runtime Peer 已显式冻结，原生依赖通过 arm64 装配和真实 PTY 往返验证。
- 0.1.5-rc.2 首次启动前会先确认全部需检查的旧版 ready 端口均已释放，再把非空 Runtime Home 复制到同级 `runtime.pre-dsh-0.1.5-rc.2[.N]` 回退目录，完成后才允许 Harness 打开 Runtime Home。新的原子事务标记不覆盖或复用既有历史回退副本；标记损坏、目标非法或并发准备未完成时失败关闭。会话日志升级为 V3，升级后不能被旧 Runtime 降级读取。
- 运行时基线测试会强制对齐 source manifest、production lock、DSH 完整性、显式 Peer 组合与全部 install-script allowlist；`runtime:verify` 会拒绝缺包或残留旧 Runtime，并验证 Node、node-pty、sharp 与 koffi。
- `runtime:vendor` 按架构装配运行时、校验 Node SHA-256、执行版本冒烟并原子替换资源目录。
- Runtime Home 与用户全局 dsh 隔离；运行时 PATH 只显式加入内置 Node/pnpm。
- `RuntimeSupervisor` 负责使用桌面壳选定的稳定回环端口、解析并校验受保护启动 URL、以一次性 token 换取 HttpOnly/SameSite Cookie、执行同源鉴权就绪探测、进程组终止和结构化失败；启动 token 与 Cookie 不进入可序列化状态或日志。
...
- 桌面端不再内置账户余额、余额 RPC、preload bridge 或账户概览开关。独立的 `dsh-deepseek-account` 插件单独注册并接管账户侧栏：当前模型为 DeepSeek 时显示 Key 余额并提供官方网页登录/充值入口；选择 Grok 或 Codex 时读取已安装 Provider 插件原有的额度 RPC，仅显示使用额度重置时间，其中 Codex 额外显示五小时额度。未安装 Grok/Codex 插件不影响 DeepSeek 功能；无法识别的自定义模型回退到 DeepSeek。Grok 与 Codex 插件本身不新增侧栏、也不改变原有能力。
- 本地 shell 与 Harness 继续使用两个独立 preload 构建产物；两侧都不再暴露余额 bridge。
...
- 当前公开版为 `v1.0.9`；`v1.0.10` 正在完成固定 `0.1.5-rc.2` Runtime、本地门禁、macOS arm64 打包/启动与 Windows 11 x64 候选生命周期验证，全部通过后才允许创建 Draft。
...
```text
Unit:        502 passed（104 files）
Integration: 50 passed / 2 platform-skipped（13 files；fake Harness、真实 0.1.5-rc.2 dsh、受管 generation fixture、内置 pnpm、发布/国内镜像与官网清单元数据契约、压力/soak 冒烟、崩溃插件隔离）
E2E arm64:   待 `v1.0.10` 打包后复核（稳定启动、完整 UI/显式退出、renderer 恢复、Session 选择恢复、Companion 宽度持久化、Workspace 搜索/筛选/预览历史与逐文件审阅、Integrated 单产品窗口/Frame 健康门）
Upgrade:     待候选门禁（上一公开版 `v1.0.9` → `v1.0.10`；真实非空 Session、相同 origin/current selection/Session 集合、0.1.5-rc.2 Runtime Home 回退副本）
...
Local package: 开发版 Runtime 已重新装配并 `runtime:verify` 通过；1.0.10 正式签名候选待完成
Artifacts:   1.0.10 双平台最终产物待正式 Release 工作流生成和验收
```
...
- 开发版装配的 `resources/runtime/dsh`（Node 24.19.0 + Harness 0.1.5-rc.2）

本地产物只适合开发验证，不应直接作为公开下载版本。当前公开版 `v1.0.9` 保持不变；`v1.0.10` 必须重新完成 macOS 签名、异机安装、真实应用 soak、Apple 公证、最终 DMG/ZIP 复验，以及 Windows 真机 Setup/portable 生命周期验证。已发布标签不可覆盖。
