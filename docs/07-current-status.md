...
- 当前公开版为 `v1.0.1`；`v1.0.2` 正在完成本地 lint、类型检查、单元测试、集成测试及固定 rc.2 Runtime 校验，macOS arm64 本机打包/启动与 Windows 11 x64 候选生命周期全部通过后才允许创建 Draft。
...
本地产物只适合开发验证，不应直接作为公开下载版本。当前公开版 `v1.0.1` 保持不变；`v1.0.2` 必须重新完成 macOS 签名、异机安装、30 分钟真实应用 soak、Apple 公证、最终 DMG/ZIP 复验，以及 Windows 真机 Setup/portable 生命周期验证。已发布标签不可覆盖。
